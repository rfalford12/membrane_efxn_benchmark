...
funnel.2nq2.m07.plot <- ggplot() +   
  geom_point( data = df.2nq2.m07, aes( x = rms, y = total_score ), color = "#e41a1c", size = 0.4 ) + 
...
funnel.2nq2.m12.plot <- ggplot() +   
  geom_point( data = df.2nq2.m12, aes( x = rms, y = total_score ), color = "#377eb8", size = 0.4 ) + 
  scale_x_continuous( "RMS (Å)", expand = c(0,0), limits = c(0, 25) ) + 
  scale_y_continuous( "Total Score", expand = c(0,0), limits = c(-800,-600) ) + 
  background_grid()

funnel.2nq2.r15.plot <- ggplot() +   
  geom_point( data = df.2nq2.r15, aes( x = rms, y = total_score ), color = "#4daf4a", size = 0.4 ) + 
  scale_x_continuous( "RMS (Å)", expand = c(0,0), limits = c(0, 25) ) + 
...
funnel.1afo.m07.plot <- ggplot() +   
  geom_point( data = df.1afo.m07, aes( x = rms, y = total_score ), color = "#e41a1c", size = 0.4 ) + 
  scale_x_continuous( "RMS (Å)", expand = c(0,0), limits = c(0, 25) ) + 
  scale_y_continuous( "Total Score", expand = c(0,0), limits = c(75, 150)) + 
  background_grid()

funnel.1afo.m12.plot <- ggplot() +   
  geom_point( data = df.1afo.m12, aes( x = rms, y = total_score ), color = "#377eb8", size = 0.4 ) + 
  scale_x_continuous( "RMS (Å)", expand = c(0,0), limits = c(0, 25) ) + 
  scale_y_continuous( "Total Score", expand = c(0,0), limits = c(0, 100) ) + 
  background_grid()

funnel.1afo.r15.plot <- ggplot() +   
  geom_point( data = df.1afo.r15, aes( x = rms, y = total_score ), color = "#4daf4a", size = 0.4 ) + 
  scale_x_continuous( "RMS (Å)", expand = c(0,0), limits = c(0, 25) ) + 
