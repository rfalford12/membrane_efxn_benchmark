...
save_plot( "~/Desktop/ddG_of_mutation_B3.tiff", composite, units = "in", base_width = 8, base_height = 2.75 )
...
OmpLA.correl.plot.new <- ggplot( data = df.OmpLA.new, aes( x = experimental_ddG, y = predicted_ddG*0.001, color = class ) ) + 
  ...
  scale_color_manual( values = c( "#e41a1c", "#377eb8", "#4daf4a", "#984ea3", "#ff7f00", "#f781bf" ) ) + 
  scale_x_continuous( "Experiment (kcal/mol)", limits = c(-4,4), expand = c(0,0) ) + 
  scale_y_continuous( "Predicted (REU)", expand = c(0,0), limits = c(-1, 1.5) ) + 
  background_grid()
...
save_plot( "~/Desktop/ddG_of_mutation_new.tiff", composite.new, units = "in", base_width = 8, base_height = 2.75 )
